...
from agent_sudo.approvals import init_approval_config, hash_passphrase, verify_passphrase, load_approval_config
...
from agent_sudo.models import ActionRequest, ApprovalRequest, ApprovalStatus, Decision, Classification
...
class PassphraseResetTests(unittest.TestCase):
    def test_first_run_setup(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertNotIn("my-new-passphrase", config_path.read_text(encoding="utf-8"))
    ...
    def test_existing_config_resets_on_accept(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            del_store = DelegationStore(delegations_path)
            # Active token
            tok_active = del_store.create(actor="act", allowed_actions=["r"], allowed_paths=["*"])
            # Already revoked token
            tok_revoked = del_store.create(actor="act", allowed_actions=["w"], allowed_paths=["*"])
            ...
            for app in approvals:
                if app.approval_request_id in {pending_app.approval_request_id, approved_app.approval_request_id}:
    ...
    def test_force_flag_bypasses_confirmation(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            init_approval_config(
                ...
                force=True,
                getpass_func=mock_getpass,
                input_func=lambda p: self.fail("input_func should not be called when force is True"),
            )
    ...
    def test_cli_integration(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("agent_sudo.approvals.getpass.getpass", return_value="cli-passphrase"), \
                ...
                argv = [
                    ...
                    "--delegations-file", str(delegations_path),
