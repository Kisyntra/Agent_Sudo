...
class ShellCommandExecutor:
    def __init__(
        self, allowed_commands: set[str] | None = None, cwd: Path | None = None
    ):
    ...
    def execute(self, request: ActionRequest) -> ExecutionResult:
        ...
        return ExecutionResult(
            request,
            placeholder_gateway_result,
            False,
            None,
            reason="missing gateway boundary",
        )
    ...
    def dry_run(self, request: ActionRequest) -> ExecutionResult:
        ...
        return ExecutionResult(
            request, placeholder_gateway_result, False, None, reason="dry-run"
        )
    ...
    def execute_with_gateway_result(
        ...
        if not argv:
            return ExecutionResult(
                request, gateway_result, False, None, reason="empty command"
            )
...
def _blocked_shell_reason(command: str) -> str | None:
    ...
    if (
        ...
        and argv[0] == "rm"
        and any(
            flag.startswith("-") and "r" in flag and "f" in flag for flag in argv[1:]
        )
    ):
    ...
    if (
        argv
        and argv[0] == "chmod"
        and any(_looks_like_auth_path(arg) for arg in argv[1:])
    ):
        return "blocked chmod against auth-related path"

    if "token" in lowered and any(
        marker in lowered
        for marker in {"http://", "https://", "curl", "wget", "upload"}
    ):
        return "blocked possible token exfiltration command"

    if any(
        marker in lowered
        for marker in {"id_rsa", ".ssh/", "aws_secret_access_key", "private_key"}
    ):
...
def _looks_like_auth_path(value: str) -> bool:
    ...
    return any(
        marker in lowered
        for marker in {".ssh", ".gnupg", "keychain", "credentials", "token", "auth"}
    )
