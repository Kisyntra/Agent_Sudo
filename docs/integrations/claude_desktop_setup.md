...
> - **Verify the audit trail**: After a tool call, run `agent-sudo audit list` to view the audit log (which defaults to the home-level path `~/.agent-sudo/mcp-audit.jsonl`). You can also specify a custom path explicitly.
